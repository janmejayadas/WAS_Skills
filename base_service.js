...
function BaseService(user_options) {
	  console.log('BaseService started: ' + user_options);
  ...
  console.log('BaseService End: ' + this._options);
}
...
BaseService.prototype.initCredentials = function(options) {
	 console.log('initCredentials Started: ' + options.token);
  ...
  if (!options.use_unauthenticated) {
    ...
    const authHeader = {
      Authorization: 'Basic ' + bufferFrom('apikey' + ':' + options.iam_apikey).toString('base64')
    };
    options.headers = extend(authHeader, options.headers);
  }
 console.log('initCredentials End: ' + options.headers);
  return options;
};
...
BaseService.prototype.getCredentialsFromEnvironment = function(name) {
	console.log('getCredentialsFromEnvironment Started: ' + name);
  name = name.toUpperCase();
   console.log('getCredentialsFromEnvironment End: ' + name);
  
  return {
    iam_apikey: process.env[name +'_IAM_APIKEY'],
...
BaseService.prototype.getCredentialsFromBluemix = function(vcap_services_name) {
	 console.log('getCredentialsFromBluemix Started: ' + vcap_services_name);
	 console.log('getCredentialsFromBluemix End: ' + vcapServices.getCredentials(vcap_services_name));
...
BaseService.prototype.getCredentials = function() {
	 console.log('getCredentials Started: ' + this._options.username);
  return {
    iam_apikey: this._options.iam_apikey,
